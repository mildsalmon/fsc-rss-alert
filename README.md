...
금감원 보도자료, 금융위 입법예고/규정변경예고, FIU 제재공시, OFAC SDN을 폴링합니다.
...
운영 상태 digest는 전일 발송 건수와 소스별 마지막 성공 시각을 `feed-ops` 채널에 보냅니다.

```bash
SLACK_BOT_TOKEN="xoxb-..." uv run python -m feed_collector digest
```
...
DIGEST_PLIST="$HOME/Library/LaunchAgents/com.mildsalmon.feed-collector-digest.plist"
cp launchd/com.mildsalmon.feed-collector-digest.plist "$DIGEST_PLIST"
plutil -remove ProgramArguments "$DIGEST_PLIST" 2>/dev/null || true
plutil -insert ProgramArguments -xml "<array><string>${REPO_DIR}/scripts/run_digest.sh</string></array>" "$DIGEST_PLIST"
plutil -replace WorkingDirectory -string "$REPO_DIR" "$DIGEST_PLIST"
plutil -replace StandardOutPath -string "$REPO_DIR/logs/launchd.digest.out.log" "$DIGEST_PLIST"
plutil -replace StandardErrorPath -string "$REPO_DIR/logs/launchd.digest.err.log" "$DIGEST_PLIST"
launchctl bootout "gui/$(id -u)" "$DIGEST_PLIST" 2>/dev/null || true
launchctl bootstrap "gui/$(id -u)" "$DIGEST_PLIST"
launchctl enable "gui/$(id -u)/com.mildsalmon.feed-collector-digest"
```
...
tail -f logs/launchd.digest.out.log logs/launchd.digest.err.log
```
...
launchctl bootout "gui/$(id -u)" ~/Library/LaunchAgents/com.mildsalmon.feed-collector-digest.plist
```

`launchd`는 20분마다 `scripts/run_poll.sh`를 실행하고, 매일 09:00 KST에 `scripts/run_digest.sh`를 실행합니다. Poll은 `sources.yaml`의 due 소스만 가져오고, 소스별 sqlite seen state로 새 글을 찾습니다. 첫 실행은 기준선만 저장하고, 이후 새 항목이 여러 개 있으면 오래된 항목부터 알림을 보냅니다. fetch, 파싱, 알림 전송 중 실패하면 해당 소스의 seen 상태를 전진하지 않고, 실패 유형을 기록합니다. 구조 변경, 차단, 로그인 필요, 404는 즉시 `feed-ops`에 알리고, 일시적 timeout/network 실패는 임계값 도달 시 한 번 알립니다. Digest는 전일 발송 건수, 마지막 성공 시각, 연속 실패 수, stale source 경고를 `feed-ops`에 1개 메시지로 보냅니다.
